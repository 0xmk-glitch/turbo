...
fs.writeFileSync(
  path.join(__dirname, '..', 'config', 'keys', 'private-key.pem'),
  privateKey,
);
fs.writeFileSync(
  path.join(__dirname, '..', 'config', 'keys', 'public-key.pem'),
  publicKey,
);
...
console.log('  - config/keys/private-key.pem (for reference)');
console.log('  - config/keys/public-key.pem (for reference)');
console.log('\n🔐 RSA Key Details:');
