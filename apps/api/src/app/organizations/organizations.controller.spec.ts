...
describe('OrganizationsController', () => {
  ...
  describe('findOne', () => {
    ...
    it('should throw error for user in different organization', async () => {
      ...
      await expect(
        controller.findOne('org-uuid-1', { user: regularUser } as any),
  ...
  describe('getUsers', () => {
    it('should return users in organization for admin', async () => {
      ...
      const result = await controller.getUsers('org-uuid-1', {
        user: adminUser,
      } as any);
    ...
    it('should throw error for non-admin user accessing different organization', async () => {
      ...
      await expect(
        controller.getUsers('org-uuid-1', { user: regularUser } as any),
      ).rejects.toThrow('Access denied');
    });
  });
});
