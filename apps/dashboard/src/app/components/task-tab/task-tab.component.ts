...
export class TaskTabComponent implements OnInit, OnDestroy {
  ...
  private registerKeyboardShortcuts(): void {
    ...
    this.keyboardShortcutService.registerShortcut({
      ...
      action: () => {
        if (
          !this.keyboardShortcutService.isInputField(document.activeElement) &&
          !this.isCreateTaskDialogOpen
        ) {
  ...
  filterTasks() {
    ...
    Object.keys(this.groupedTasks).forEach((category) => {
      Object.keys(this.groupedTasks[category]).forEach((status) => {
        ...
        this.filteredTasks[category][statusNum] = this.groupedTasks[category][
          statusNum
        ].filter((task) => {
          // Priority filter
          const priorityMatch =
  ...
  private updateTask(taskData: CreateTaskDto) {
    ...
    Object.keys(this.groupedTasks).forEach((category) => {
      Object.keys(this.groupedTasks[category]).forEach((status) => {
        const statusNum = parseInt(status);
        const taskIndex = this.groupedTasks[category][statusNum].findIndex(
